...
const RequestsButton = () => {
  ...
  React.useEffect(() => {
    ...
    const lastMinuteRequests = all.filter(
      (r) => r.start && r.start > lastMinute
    );
    const lastMinTokens = lastMinuteRequests.reduce(
      (acc, r) => acc + getTokenCount(r.prompt + " " + r.context),
      0
    );
  ...
  return (
    <>
      {showThrottleMessage ? (
        <Text color="orange" fontWeight="800">
          Tokens/Minute limit reached. Waiting {waitTime} seconds for next
          batch...
        </Text>
      ) : null}
      <Stack align="center">
      ...
      <Box
        ...
        justify="center"
      >
        <Text color="lime" fontSize="12px" fontWeight="800">
          Tokens in Last Minute
        </Text>
        <Text color="lime" fontSize="18px" fontWeight="300">
          {lastMinuteTokens} / {rateLimit} TPM
        </Text>
      </Box>

      <Stack>
        <Badge colorScheme="yellow">{pendings.length} Pending</Badge>
